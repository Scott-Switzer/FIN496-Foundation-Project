...
- Net annualized return is 14.62% for `SAA+TAA` versus 8.83% for `SAA` and 7.86% for `BM2`. [Source: `taa_project/outputs/portfolio_metrics.csv`]
- Net Sharpe improves by 0.50 versus `BM2`, while the Deflated Sharpe Ratio is 0.999 across 116 disclosed TAA trials. [Sources: `taa_project/outputs/portfolio_metrics.csv`, `taa_project/outputs/dsr_summary.csv`, `TRIAL_LEDGER.csv`]
...
## Performance Results
- `SAA+TAA` delivers 14.62% annualized return, 11.34% annualized volatility, and -27.95% max drawdown. [Source: `taa_project/outputs/portfolio_metrics.csv`]
- Relative to `SAA`, the TAA overlay changes annualized return by 5.79% and cost drag by 0.19% per year. [Source: `taa_project/outputs/portfolio_metrics.csv`]
...
- This run achieved -27.95% maximum drawdown against the IPS tolerance of -25%. The submission-selection summary was not available in this output directory.
