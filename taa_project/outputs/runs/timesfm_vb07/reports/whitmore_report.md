...
- Net annualized return is 5.71% for `SAA+TAA` versus 8.83% for `SAA` and 7.86% for `BM2`. [Source: `taa_project/outputs/portfolio_metrics.csv`]
- Net Sharpe improves by 0.11 versus `BM2`, while the Deflated Sharpe Ratio is 0.850 across 246 disclosed trials in `TRIAL_LEDGER.csv`. [Sources: `taa_project/outputs/portfolio_metrics.csv`, `taa_project/outputs/dsr_summary.csv`, `TRIAL_LEDGER.csv`]
...
## Performance Results
- `SAA+TAA` delivers 5.71% annualized return, 5.12% annualized volatility, and -16.63% max drawdown. [Source: `taa_project/outputs/portfolio_metrics.csv`]
...
- This run achieved -16.63% maximum drawdown against the IPS tolerance of -25%. The submission-selection summary was not available in this output directory.
