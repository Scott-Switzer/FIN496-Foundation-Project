...
def test_simulate_portfolio_triggers_compliance_rebalance(tmp_path) -> None:
    ...
    weights_df, returns_df = simulate_portfolio(
        ...
        start_date=start,
        end_date=next_day,
        compliance_log_path=tmp_path / "saa_compliance_rebalances.csv",
    )
    ...
    log = pd.read_csv(tmp_path / "saa_compliance_rebalances.csv")
    assert {"date", "rule", "pre_trade_value", "bound", "turnover", "remediation"}.issubset(log.columns)
