...
def test_run_pipeline_orchestrates_with_stubbed_dependencies(monkeypatch, tmp_path) -> None:
    ...
    def fake_run_walkforward(start, end, folds, use_timesfm, vol_budget, output_dir, ensemble_config=None):
        ...
        captured["walkforward_daily_risk_governor"] = False if ensemble_config is None else ensemble_config.use_daily_risk_governor
        return {
            "folds": pd.DataFrame(),
    ...
    def fake_build_reporting(start, end, folds, use_timesfm, vol_budget, output_dir, figure_dir, saa_method="min_variance", ensemble_config=None):
        ...
        captured["reporting_daily_risk_governor"] = False if ensemble_config is None else ensemble_config.use_daily_risk_governor
    ...
    assert captured["walkforward_daily_risk_governor"] is True
    assert captured["attribution_daily_risk_governor"] is True
    assert captured["reporting_daily_risk_governor"] is True
