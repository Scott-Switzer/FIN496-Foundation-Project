...
DAILY_RISK_VOL_TRIGGER = 0.09
DAILY_RISK_VOL_RELEASE = 0.063
...
def _daily_risk_should_be_defensive(currently_defensive: bool, realized_returns: list[float]) -> bool:
    current_drawdown, realized_vol = _daily_realized_risk_state(realized_returns)
    if currently_defensive:
        return not (current_drawdown >= DAILY_RISK_DD_RELEASE and realized_vol <= DAILY_RISK_VOL_RELEASE)
...
def simulate_period_returns(
    ...
    if bool(risk_state.get("active", False)):
    ...
    for offset, date in enumerate(period_dates):
        ...
        next_risk_active = _daily_risk_should_be_defensive(bool(risk_state.get("active", False)), realized_returns)
        ...
        if next_risk_active != bool(risk_state.get("active", False)):
            if next_risk_active:
                ...
                target = starting_weights.reindex(ALL_TAA).fillna(0.0)
