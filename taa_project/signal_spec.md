...
sources (macro regime, price-trend, cross-sectional momentum, asset-specific
macro factors), plus a small VIX/yield-curve trip-wire. The signals are
designed to be **imperfectly correlated**, so
...
| **4. Asset-specific macro factor** | Real yields, credit spreads, and crypto trend help distinguish inflationary stress from deflationary stress and avoid treating all macro drawdowns as bond rallies. | Erb & Harvey 2013; Gilchrist & Zakrajsek 2012 |
| **VIX/yield-curve trip-wire** | A fast, auditable stress overlay reacts to VIX spikes faster than the monthly HMM. The yield curve is a sizing penalty for positive risk-on scores, not a standalone forward-looking volatility input. | FRED `VIXCLS`; FRED `T10Y3M` |
